...
This is the ordinary baseline and does not require every documentation, UI, or small App change to build Canary. If a change affects STCP backend selection, the pure Kotlin backend, `AppContainer` STCP assembly, shared manager integration, or CI/Release variant validation, also run `:app:testCanaryUnitTest` and `:app:assembleCanary`.
...
Canary tasks are conditional for ordinary work, not a universal minimum. They are required for STCP backend/selection changes and are always part of the complete bridge/CI-equivalent gate.
