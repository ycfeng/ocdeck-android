...
- Every `OpenCodeApi` Retrofit method must declare an explicit inbound response policy. Ordinary successful JSON/object/list/Boolean/`JsonElement` responses and the transparently decompressed `/file/content` entity are bounded to 16 MiB; successful `Response<Unit>` bodies are discarded.
- The Retrofit inbound interceptor fails closed when a policy is missing. It uses `Content-Length` only for early rejection, enforces unknown or understated lengths at `max + 1`, and closes and replaces every non-2xx body with an empty body before Retrofit can cache it while preserving the status code. Requests without a Retrofit `Invocation`, including the direct session-message transport, are outside this interceptor.
...
- Limit each physical/logical SSE line and accumulated event data to 32 MiB of UTF-8 wire bytes. Enforce line limits before conversion to `String` and event limits before complete concatenation. On overflow, close the body, cancel the Call, and report a typed failure without payload content.
...
- Decode a 2xx session-message body on the OkHttp callback thread with a counting/limiting `InputStream` and the shared tolerant `Json.decodeFromStream`. `Content-Length` is only an early rejection; fail on wire byte `max + 1`, then verify EOF through the same limiter. The complete UTF-8 wire limit is 64 MiB. Coroutine cancellation must cancel the Call and close the active body without replacing `CancellationException` with an I/O error.
- The 16 MiB Retrofit, 64 MiB session-message, and 32 MiB SSE limits are wire-byte limits, not heap-usage guarantees. Retrofit converters may still materialize a complete `String`, DTO graph, or JSON tree within the limit, and concurrent project snapshots can still create a high memory peak. `/file/content` retains its reader-level second defense.
