...
- Canary and Kotlin Release-Like now resolve bridge-free library variants; classpath and APK packaging gates reject the GoMobile bridge and `libgojni.so`, while Debug/Release and Android A/B interoperability retain the real bridge. / Canary 与 Kotlin Release-Like 现在解析无 bridge 的 library variant；classpath 与 APK 打包门禁会拒绝 GoMobile bridge 和 `libgojni.so`，而 Debug/Release 与 Android A/B 互操作仍保留真实 bridge。
