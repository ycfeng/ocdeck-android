...
5. Run the bridge AAR gate, App Debug, Canary, and Kotlin Release-Like unit tests, `:frpc-stcp-visitor` unit tests, and all three verification APK builds.

The Debug factory test confirms the GoMobile default, while the Canary and Kotlin Release-Like factory tests confirm pure Kotlin selection. Because both implementations live in the same Android library, either Kotlin verification APK may still package Go native bytes; the gate validates actual App assembly rather than asserting native-byte absence.
