...
- `.github/workflows/frpc-kotlin-android-interop.yml` is a manual, exact-SHA K6V gate for the Android STCP backend. It runs the real GoMobile AAR and pure Kotlin client sequentially on API 26 and API 36 x86_64 emulators, remains read-only, and produces a bounded bilingual acceptance-report artifact without signing material.
...
The manual K6V workflow requires its lowercase 40-character `candidate_sha` input, `github.sha`, `github.workflow_sha`, and checked-out `HEAD` to identify the same commit. Each emulator lane builds the real GoMobile bridge first, then runs `:frpc-stcp-visitor:frpcAndroidInteropTest` in a fresh Gradle invocation. GoMobile and Kotlin execute in separate instrumentation processes; GoMobile must stop, report `Closed`, and release the listener before Kotlin reuses the same bind port. Phase one covers forced TLS, `/global/health`, global/project SSE, two echo transfers, and two larger-than-flow-window downloads with wire v1 and payload encryption/compression disabled. Report status is bound to the complete matrix result, and each lane records the actual Android test APK and GoMobile bridge AAR SHA-256 values. The resulting artifact contains English and Chinese reports, normalized evidence JSON, and `SHA256SUMS`, but no credentials, endpoints, private paths, raw process logs, or test configuration.

Neither ordinary CI nor the K6V workflow possesses a JKS, passwords, or repository write permission. Debug and Canary APKs remain verification outputs only. The host interop job covers the complete wire/payload, negative, and restart matrix, while K6V adds real Android backend execution. Its x86_64 emulator evidence does not replace physical target-ABI native loading, a 16KB page-size device, Doze or network-transition checks, foreground/background behavior, performance, soak, or release-device STCP validation. A passing K6V artifact is evidence for evaluating K7; it does not itself authorize Kotlin-default assembly.
...
- Keep the manual K6V Android interop workflow at `contents: read`, outside every Environment, and without repository or Environment secrets. Require an exact candidate SHA and retain its report artifact as candidate-specific evidence; do not treat it as an ordinary required check while it remains manual and path-independent.
...
- Android K6V interoperability fails: verify exact-SHA identity, the pinned API 26/API 36 x86_64 system images, bridge generation, emulator API/ABI, and the structured lane result, then reproduce with `:frpc-stcp-visitor:frpcAndroidInteropTest -PrequireGoMobileBridge=true -Pocdeck.frp.androidInterop.deviceSerial=<serial>`. Do not pass credentials through Gradle or instrumentation arguments, relax private-file transport or ownership-aware cleanup, or accept a partial/missing lane as a passing report.
