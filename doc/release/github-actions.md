...
Neither ordinary CI nor the K6V workflow possesses a release JKS, passwords, or repository write permission. Debug, Canary, and Kotlin Release-Like APKs remain verification outputs only; Kotlin Release-Like uses only the standard Android Debug test certificate. The host interop job and the API 36 K6V `full` lane independently cover the complete wire/payload, negative, and restart matrix at different execution boundaries. The fixed x86_64 emulator evidence does not replace the split target-ABI physical-device and 16KB runtime lanes, the App's real Store/snapshot/reconciliation path, Doze or network-transition checks, foreground/background behavior, performance, resource-leak and long-duration soak evidence, a formal stable release cycle, or release-device STCP validation. The two device lanes may use different devices: physical ARM covers target-ABI installation/class loading and real App/STCP behavior, while an official Android 16KB test image or physical device covers runtime page size, startup, and applicable interoperability with static alignment/packaging gates. Missing physical ARM 16KB hardware is a disclosed residual risk, not a K7 development blocker. A passing K6V artifact is evidence for evaluating K7; it does not itself authorize Kotlin-default assembly.
...
3. Complete the [release checklist](checklist.md), including target-ABI physical-device native load, 16KB runtime validation on an official Android test image or physical device, and real STCP validation. These dimensions may use different devices; a missing required dimension still blocks a formal release.
...
- A required target-ABI physical-device, 16KB runtime, or real STCP gate fails or is incomplete: do not publish. The lanes may use different devices; fix and repeat the affected gate or defer the release.
