...
- [ ] If this candidate is being evaluated for K7/Kotlin-default assembly, `K6V Android STCP Interop` ran against its exact full commit SHA, either by direct manual dispatch or by the explicitly enabled manual CI reusable bootstrap during first introduction; both API 26 and API 36 x86_64 lanes passed and the bilingual acceptance report is `Pass`. Otherwise this item is explicitly recorded as not applicable.
