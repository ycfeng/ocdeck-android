...
The explicitly requested K6V workflow provides an x86_64 emulator gate for STCP backend interoperability, but ordinary CI still has no App UI emulator/instrumentation job. K6V can be dispatched directly after its workflow exists on the default branch; while the workflow is first being introduced, a manual CI dispatch can opt into the same branch-local reusable workflow. Push and pull-request CI never enables that bootstrap automatically. The `app/src/androidTest` suite covers localized independent-window roots. Recent-project drag ordering remains outside instrumentation coverage; its automated coverage is primarily the JVM reducer, recorder, ViewModel, and drawer-model tests listed below. Project selection, session navigation, broader Composer interactions and pickers, permission/question UI, large-text behavior, and TalkBack still need systematic device automation.
...
- `FrpcStcpVisitorAndroidInteropTest`, launched through `frpcAndroidInteropTest`, runs the real generated GoMobile AAR and the pure Kotlin backend in separate instrumentation processes. It fully stops the first session before the second backend reuses the same bind port, then verifies public readiness/state semantics, `/global/health`, global/project SSE, two concurrent echoes, two larger-than-window downloads, terminal close, and port release without an active-generation fallback.
...
Build the generated GoMobile AAR first, then start a new Gradle invocation so the configuration-time bridge dependency is present. With exactly one authorized emulator/device, or with an explicit serial, run:
...
GoMobile runs first in a fresh instrumentation process. After `stopVisitor`, `stopSession`, terminal-state verification, and listener-port release, Kotlin runs in a second process and must rebind the same port. The phase-one device scenario is intentionally `wire=v1`, encryption off, and compression off; the host JVM harness remains responsible for wire v1/v2, all four payload modes, typed negative cases, and restart recovery. Device evidence must record API level, ABI, and page size. This gate does not cover performance, soak, Doze, foreground/background transitions, network switching, physical arm devices, or 16KB page-size hardware.

The explicitly requested `.github/workflows/frpc-kotlin-android-interop.yml` runs the exact candidate SHA on API 26 and API 36 x86_64 emulators. It supports direct `workflow_dispatch` and a read-only `workflow_call`; the latter is used only when a manual CI dispatch sets `run_frpc_android_interop=true`, which bootstraps the branch-local workflow before it exists on the default branch. The optional CI `candidate_sha` defaults to the dispatched ref SHA, and push/pull-request CI never invokes K6V. Report status is bound to the complete matrix result, and each lane records the actual Android test APK and GoMobile bridge AAR SHA-256 values. It uploads bounded English/Chinese acceptance reports, consolidated JSON evidence, and `SHA256SUMS`; it has read-only repository permission, no signing Environment or secrets, and does not authorize switching the formal backend default. Physical-device and long-running K6V evidence remain separate requirements.

Recorded phase-one evidence: exact candidate `459c2b57ebf465d6b933ea939f59fa739128ec59` (tree `a6ce1019adfa351bc15e09bc479220a967bdf323`) passed [workflow run 29716724485](https://github.com/ycfeng/ocdeck-android/actions/runs/29716724485). The API 26 Android 8.0.0 and API 36 Android 16 x86_64 emulator lanes both reported 4 KiB pages and completed the GoMobile-then-Kotlin scenario. The consolidated artifact `k6v-frpc-android-interop-459c2b57ebf465d6b933ea939f59fa739128ec59` reported `Pass`; its GitHub artifact digest is `sha256:6c074ac7d7d6a7a5584f8bd9e6e90fa1746bbe5ff5ef226b1acfc8f7bc532b28`. This historical record applies only to that exact candidate and phase-one scope; it does not satisfy physical ARM/16KB, expanded Android wire/payload/restart, Doze/network/foreground, performance, resource-leak, or soak gates and does not authorize Kotlin-default assembly.
