...
Canonical STCP 契约 fixture 覆盖共享 bridge DTO JSON，以及固定版本的 wire、control transport 和 yamux 契约字节。固定 Go oracle 位于 `frpc-stcp-visitor-go/internal/contractfixture/`；`manifest.json` 记录固定输入、provenance、限制、路径、长度、哈希、预期语义、分块方案和 mutation recipe。
