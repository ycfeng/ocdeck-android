...
`:frpc-stcp-visitor:frpcInteropTest` 与 `:frpc-stcp-visitor:frpcAndroidInteropTest` 使用的官方 frp executable 是测试工具，不是已提交 fixture 或项目分发 asset；两个显式任务共用同一个 provisioner。`third_party/sources/frp.json` 为 Linux、Windows、macOS 的 amd64/arm64 固定精确 v0.69.1 archive URL 与 SHA-256。任务只把当前主机 asset 下载到 Gradle cache，在有界安全解压前重新校验 archive，并验证两个 executable 的版本。不得提交或上传下载的 archive、executable、生成的 TLS 材料、临时配置、原始设备/进程日志或私有调用文件。
