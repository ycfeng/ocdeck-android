...
| 层级 | 当前覆盖 |
| --- | --- |
| Kotlin/JVM 单元测试 | 路径与项目文件 URL、脱敏、Retrofit/direct/SSE 入站边界、类型化失败与本地化 UI 映射、安全 value 摘要、DTO 容错、Store revision、prompt/项目上下文状态与恢复、Provider auth/OAuth 与分阶段 custom-config 事务、服务器凭据事务、SSH/STCP 协调、对比度和 feature helper。 |
...
- `RetrofitInboundResponsePolicyTest` 验证每个 `OpenCodeApi` 方法声明 `BOUNDED` 或 `EMPTY_SUCCESS`；缺少策略时在网络请求前失败；已声明、未知和低报长度都在 `max + 1` 执行 16 MiB 边界；非 2xx 与成功 Unit body 不读取即关闭；`/file/content` 保持延迟读取；没有 Retrofit `Invocation` 的请求绕过该拦截器。
- `SessionMessagesTransportTest`、`SessionMessagesResponseReaderTest`、`FileContentResponseReaderTest` 覆盖无 body 的非 2xx 失败、OkHttp callback 线程 direct decode、精确/未知/低报长度、EOF 验证、取消与 callback race、session messages 64 MiB 边界，以及 `/file/content` reader 层纵深防御。
...
- `BoundedSseReaderTest`、`OkHttpSseEventSourceFactoryTest`、`OpenCodeEventClientLifecycleTest`、`ProjectSnapshotCoordinatorTest` 覆盖线缆字节解析、所有换行与 EOF 状态、32 MiB 行/event 边界、无 body status/MIME 失败、取消、重试分类、关闭终态、owner/generation/source/transport 竞态、project/global 权威切换、有界重放和快照失败/恢复。
...
- `ProjectDrawerModelTest` 与 `ProjectInitialTest` 覆盖当前项目补入、MRU 顺序保持、Windows 路径别名去重、项目首页导航决策和 Unicode 项目首字。
...
- 对 Retrofit 方法验证显式入站模式、精确上限、`max + 1`、未知与低报长度、body 关闭行为、非 2xx 零读取、成功 Unit body 丢弃和无 `Invocation` 绕过。
