...
显式请求的 `.github/workflows/frpc-kotlin-android-interop.yml` 会对精确候选 SHA 固定运行 API 26 x86_64 `compat` 与 API 36 x86_64 `full`。它同时支持直接 `workflow_dispatch` 与只读 `workflow_call`；后者仅在手动 CI dispatch 设置 `run_frpc_android_interop=true` 时使用，用于该 workflow 尚未进入默认分支时调用分支内版本。CI 的可选 `candidate_sha` 默认采用所触发 ref 的 SHA，push/PR CI 绝不会调用 K6V。Preflight 会运行报告 renderer 单元测试。Renderer 只有在两份 lane 文件及其 Host summary 都满足精确 lane/profile 契约时才通过，否则 fail closed；它不会从 Gradle outcome 推断 profile 成功。每个 lane 还会记录实际 Android test APK 与 GoMobile bridge AAR 的 SHA-256。Workflow 会上传有界的中英文验收报告、合并 JSON 证据和 `SHA256SUMS`；它只有仓库只读权限，不使用签名 Environment 或 secret，并且每份报告都保持 `authorizesKotlinDefault=false`。扩展自动化已实现，但尚未记录新的精确 SHA API 26/API 36 workflow artifact，因此当前 `full` suite 仍等待候选门禁证据。

已记录的历史第一阶段证据：精确候选 `459c2b57ebf465d6b933ea939f59fa739128ec59`（tree `a6ce1019adfa351bc15e09bc479220a967bdf323`）已通过 [workflow run 29716724485](https://github.com/ycfeng/ocdeck-android/actions/runs/29716724485)。API 26 Android 8.0.0 与 API 36 Android 16 的 x86_64 emulator lane 均报告 4 KiB page，并完成旧的单一 GoMobile 后 Kotlin 第一阶段场景。合并 artifact `k6v-frpc-android-interop-459c2b57ebf465d6b933ea939f59fa739128ec59` 状态为“通过”，其 GitHub artifact digest 为 `sha256:6c074ac7d7d6a7a5584f8bd9e6e90fa1746bbe5ff5ef226b1acfc8f7bc532b28`。这仍然只是该精确候选与旧第一阶段范围的历史证据，不能证明当前 API 26 `compat`/API 36 12-profile `full` workflow，也不能满足物理 ARM/16KB、App 的真实 Store/快照/reconciliation 链路、Doze/网络/前后台、性能、资源泄漏、长期 soak 或正式稳定发布周期门禁，更不授权 Kotlin 默认装配。
