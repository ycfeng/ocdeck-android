...
显式请求的 `.github/workflows/frpc-kotlin-android-interop.yml` 会对精确候选 SHA 固定运行 API 26 x86_64 `compat` 与 API 36 x86_64 `full`。它同时支持直接 `workflow_dispatch` 与只读 `workflow_call`；后者仅在手动 CI dispatch 设置 `run_frpc_android_interop=true` 时使用，用于该 workflow 尚未进入默认分支时调用分支内版本。CI 的可选 `candidate_sha` 默认采用所触发 ref 的 SHA，push/PR CI 绝不会调用 K6V。Preflight 会运行报告 renderer 单元测试。Renderer 只有在两份 lane 文件及其 Host summary 都满足精确 lane/profile 契约时才通过，否则 fail closed；它不会从 Gradle outcome 推断 profile 成功。Host 与 lane 证据均通过 `adb shell getconf PAGE_SIZE` 读取 Android runtime ABI page size，从而避免把 x86_64 16 KiB Android 测试镜像按 `/proc/self/smaps` 报告的 4 KiB MMU backing page 错误分类。每个 lane 还会记录实际 Android test APK 与 GoMobile bridge AAR 的 SHA-256。Workflow 会上传有界的中英文验收报告、合并 JSON 证据和 `SHA256SUMS`；它只有仓库只读权限，不使用签名 Environment 或 secret，并且每份报告都保持 `authorizesKotlinDefault=false`。
