...
活动 listener 对 bind port 保持独占。旧 generation 完整停止后，Kotlin loopback listener 只启用 address reuse，不启用 `SO_REUSEPORT`，因此已完成 relay 的连接仍处于 `TIME_WAIT` 时，新 generation 也能快速重绑同一端口。执行 `stopVisitor` 时，Kotlin runtime 等待目标 visitor 的本地 listener 与 relay I/O 停止，但延迟的 best-effort Yamux reset 继续由 session-owned watcher 收口；该 watcher 在 reset 完整结束前继续持有 stream permit。`stopSession` 会在进入终态前 join 该 watcher，并观察迟到的取消或 JVM `Error`。该行为不允许 backend 或 generation 重叠运行，不能把 `stopVisitor` 视为完整 wire-reset 屏障，也不是运行时 fallback 机制。
...
- `FrpcStcpVisitorAndroidInteropTest` 由 `frpcAndroidInteropTest` 驱动：真实 GoMobile AAR 与纯 Kotlin client 分别在独立 Android instrumentation 进程中运行，只有完整停止后才顺序复用同一个 bind port，并验证公共 revision/epoch/listener 状态、`/global/health`、global/project SSE、并发 echo/超窗口流量、`Closed` 和端口释放，不存在活动 generation fallback。
...
- Android A/B 验证必须先在独立 invocation 中构建 GoMobile AAR，再运行 `./gradlew --no-daemon :frpc-stcp-visitor:frpcAndroidInteropTest -PrequireGoMobileBridge=true -Pocdeck.frp.androidInterop.deviceSerial=<serial>`。它复用固定官方 frp provisioner，并通过有界 adb stdin 将合成凭据写入测试私有文件，而不是通过 Gradle 或 instrumentation argument 传递。
...
- `.github/workflows/ci.yml` 在 `main` push/PR 上执行固定 frp host 互操作 job，以及不使用 Release 签名 secret 的 Debug 与 Canary 门禁；`.github/workflows/frpc-kotlin-android-interop.yml` 是手动、只读、精确 SHA 的 API 26/API 36 x86_64 Android STCP 门禁，生成有界报告 artifact，且不授权 K7；`.github/workflows/release.yml` 在 release Environment 之外的独立只读 job 中运行 host 互操作，再重跑两个验证 variant，并仅对稳定 tag 的 GoMobile 默认 Release 制品应用 Release 签名。
