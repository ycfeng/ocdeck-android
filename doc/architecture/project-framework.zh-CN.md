...
聚焦的 `app/src/androidTest` 测试集已覆盖本地化独立窗口根，但 CI 仍没有 emulator/instrumentation job。在更广泛的设备自动化建立前，需要在紧凑手机屏幕、浅色/深色主题、IME 打开、200% 字体和 TalkBack 启用条件下人工验证以下交互：
...
- `.github/workflows/ci.yml` 在 `main` push/PR 上执行固定 frp 互操作 job，以及不使用 Release 签名 secret 的 Debug 与 Canary 门禁；`.github/workflows/release.yml` 在 release Environment 之外的独立只读 job 中运行互操作，再重跑两个验证 variant，并仅对稳定 tag 的 GoMobile 默认 Release 制品应用 Release 签名。
...
- 纯 Kotlin payload encryption/compression、跨语言 fixture 和固定官方 frp host 专用门禁均已实现。该门禁覆盖 wire v1/v2、四种 payload 组合、并发双向超窗口流、REST 与同时存在的 global/project SSE、类型化认证/secret/bind 失败、TLS、既有 SSE 中断及重启/control epoch 恢复。Android socket/TLS 行为、长期 soak 和发布设备证据仍是独立真机门禁。
...
1. 在 Android 设备重复固定 frp host 矩阵中的关键 wire v1/v2、payload、并发 REST/SSE 与重启/control epoch 路径，并收集长期 soak 与资源证据，再评估是否进入 Kotlin 默认装配。
