...
- Pure Kotlin payload encryption/compression, cross-language fixtures, the dedicated fixed official-frp host gate, and the expanded real Android A/B automation are implemented. The Android harness now fixes API 26 to `compat` with only `success-v1-00` and API 36 to `full` with the exact 12-profile wire/payload/negative/restart matrix. For every profile it runs GoMobile, verifies complete stop/`Closed`/port release, then runs Kotlin in a separate instrumentation process on the same port; success profiles keep global/project SSE online during health, two echo requests, and two larger-than-Yamux-window downloads with a shared checkpoint, while restart verifies existing-SSE interruption, control unavailability, `frps` and provider recovery barriers, a strictly advanced control epoch, and recovered REST/SSE/large traffic. Strict Host summaries prove profile parameters, backend order, checks, and equivalence rather than treating Gradle success as evidence. This automation still awaits a new exact-candidate API 26/API 36 workflow artifact. Candidate `459c2b57ebf465d6b933ea939f59fa739128ec59` and [run 29716724485](https://github.com/ycfeng/ocdeck-android/actions/runs/29716724485) remain only old phase-one historical evidence and do not prove the current `full` suite. Physical ARM and 16KB devices, the App's real Store/snapshot/reconciliation path, Doze/network and foreground/background transitions, performance, resource-leak evidence, long-duration soak, and formal stable release cycles remain separate gates.
...
1. Run the implemented fixed API 26 `compat`/API 36 `full` K6V workflow against a new exact candidate and review its complete fail-closed structured artifact; the old phase-one artifact does not satisfy this gate. Then validate physical target ABIs and 16KB devices, the App's real Store/snapshot/reconciliation path, Doze/network and foreground/background transitions, performance, resource leaks, long-duration soak, and formal stable release cycles before considering Kotlin-default assembly.
