...
1. Using the recorded exact-SHA API 26 `compat`/API 36 `full` K6V pass as the emulator baseline, validate physical target ABIs and 16KB devices, the App's real Store/snapshot/reconciliation path, Doze/network and foreground/background transitions, performance, resource leaks, long-duration soak, and formal stable release cycles before considering Kotlin-default assembly.
