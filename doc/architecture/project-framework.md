...
- Both implementations live in `:frpc-stcp-visitor`, so Canary and Kotlin Release-Like APKs may still package Go native libraries. The guarantee is which client instance `AppContainer` selects, not the absence of the other implementation's packaged bytes.
...
Android Debug still compiles when the AAR has not been generated, but its GoMobile-backed STCP path returns a clear runtime unavailable error. Canary and Kotlin Release-Like select the Kotlin backend, although the shared library may still package Go native bytes when the AAR exists. Formal Release `preReleaseBuild` must run `checkGoMobileBridgeAar`, byte-for-byte verifying checksum, pinned API signature, internal/external bridge and frp provenance, legal texts, licenses, native metadata, and each ABI's `libgojni.so` hash. `-PrequireGoMobileBridge=true` applies the same AAR gate to the full Debug/Canary/KotlinRelease CI verification. Passing static gates is not sufficient for formal release: GoMobile native load and a real STCP loop must still be verified on target ABIs and 16 KB page-size devices, while the non-publishable Kotlin Release-Like APK provides a separate Release-mode device-validation path without changing the formal backend default.
...
- Store maintains separate project-data and message-data revisions. Direct ViewModel REST project snapshots use revision CAS and a latest-request gate, and cannot overwrite later SSE/coordinator state or a newer refresh. A message GET replaces at equal revision while preserving local optimistic messages and parts omitted by REST; when revision changed, merge by message/part ID while preserving realtime fields, deltas, historical completion, and deletion tombstones created during the request. Rebuild empty message text from final effective text parts. `session.deleted` removes that session's messages/parts and advances deletion revision. Connection/loading/notification changes do not advance project-data revision.
...
- The full bridge/CI Android gate is `./gradlew :frpc-stcp-visitor:checkGoMobileBridgeAar :app:testDebugUnitTest :app:testCanaryUnitTest :app:testKotlinReleaseUnitTest :frpc-stcp-visitor:testDebugUnitTest :app:assembleDebug :app:assembleCanary :app:assembleKotlinRelease -PrequireGoMobileBridge=true`.
...
1. Using the recorded exact-SHA API 26 `compat`/API 36 `full` K6V pass as the emulator baseline, use the available non-publishable Kotlin Release-Like APK to validate physical ARM target ABIs and 16KB devices, Release-mode class loading, the App's real Store/snapshot/reconciliation and recovery paths, Doze/network and foreground/background transitions, performance, resource leaks, and long-duration soak. Continue formal stable-release-cycle validation separately; this verification APK is not eligible for publication and does not authorize Kotlin-default assembly.
