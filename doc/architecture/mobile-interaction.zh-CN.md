...
| STCP 并发与恢复 | 主体具备 | readiness、generation/control epoch、AAR 门禁和扩展后的真实 Android GoMobile→Kotlin A/B 自动化均已实现。固定 API 26 x86_64 `compat` lane 只含 `success-v1-00`；API 36 x86_64 `full` lane 包含精确 12-profile wire/payload/negative/restart 矩阵，并由严格 Host summary 校验 backend 顺序、参数、checks 与等价性。这两条 lane 仍等待新的精确 SHA workflow artifact。候选 `459c2b57ebf465d6b933ea939f59fa739128ec59` 与 run 29716724485 仅是旧第一阶段历史证据，不能证明当前 `full` suite；仍需物理目标 ABI、16KB 设备、App 的真实 Store/快照/reconciliation 链路、Doze/网络与前后台切换、性能、资源泄漏、长期 soak 和正式稳定发布周期证据 |
...
| 优先级 | 内容 |
| --- | --- |
| 高 | 获取并审查已实现 API 26 `compat`/API 36 `full` K6V workflow 的新精确 SHA 通过证据，随后验证物理 ARM/16KB 设备、App 的真实 Store/快照/reconciliation 链路、Doze/网络与前后台切换、性能、资源泄漏、长期 soak 和正式稳定发布周期；完成前不得作出 Kotlin 默认决策 |
