...
| STCP 并发与恢复 | 主体具备 | readiness、generation/control epoch、AAR 门禁和扩展后的真实 Android GoMobile→Kotlin A/B 自动化均已实现。固定 API 26 x86_64 `compat` lane 只含 `success-v1-00`；API 36 x86_64 `full` lane 包含精确 12-profile wire/payload/negative/restart 矩阵，并由严格 Host summary 校验 backend 顺序、参数、语义结果、checks 与等价性。精确候选 `c474ccd18769544a3d7a08facf4147dfe6305bee` 已在 [run 29754014231](https://github.com/ycfeng/ocdeck-android/actions/runs/29754014231) 通过两条 lane 及 fail-closed 双语报告。仍需物理目标 ABI、16KB 设备、App 的真实 Store/快照/reconciliation 链路、Doze/网络与前后台切换、性能、资源泄漏、长期 soak 和正式稳定发布周期证据；emulator 报告不授权 Kotlin 默认装配 |
...
| 优先级 | 内容 |
| --- | --- |
| 高 | 以已记录的精确 SHA API 26 `compat`/API 36 `full` K6V 通过结果作为 emulator 基线，验证物理 ARM/16KB 设备、App 的真实 Store/快照/reconciliation 链路、Doze/网络与前后台切换、性能、资源泄漏、长期 soak 和正式稳定发布周期；完成前不得作出 Kotlin 默认决策 |
