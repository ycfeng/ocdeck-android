...
目标是为 OC Deck 复刻 OpenCode Web UI 核心工作流提供可落地的页面、交互、状态、接口和移动端适配设计。OC Deck 是独立社区客户端，不是 OpenCode 项目或 Anomaly 官方产品；应用 `0.2.3` 使用 `io.github.ycfeng.ocdeck`，最低支持 Android API 26，并要求用户自行提供可访问的 OpenCode Server。本文不包含任何真实 provider API key、token、密码或本机凭据；所有配置接口中的敏感字段都必须在日志、文档、崩溃上报和调试 UI 中脱敏为 `<redacted>`。
...
当前原生实现：`ProjectPickerScreen` 顶部保留标题 `打开项目`，左上角 `服务器` 按钮进入服务器管理页，右上角 `设置` 按钮进入设置页；顶部不展示返回按钮或打开按钮，打开动作保留在页面主体，页面底部不再提供额外“返回”按钮。路径候选目录项点击会将规范化路径补全到输入框、自动追加 `/` 并继续查询该目录下的候选路径，光标自动移到补全路径末尾，但不直接进入项目；用户需点击主体“打开项目”按钮进入。最近项目卡片正文点击打开项目且不改变已保存位置，右侧删除图标只移除本机最近项目记录，也绝不启动拖拽；删除前使用 OpenCode 风格二次确认弹窗，文案需明确不会删除服务端项目文件。只有长按卡片正文才启动带边缘自动滚动的纵向重排；项目选择页先乐观更新，再持久化共享顺序，持久化失败时恢复已保存顺序。
...
项目选择页和 Android rail 订阅同一份按服务器持久化的顺序，每个服务器最多 20 个项目。每条 DataStore 序列化记录都有数值 `sortOrder`，界面按数值升序展示。旧记录缺少 `sortOrder` 时保留其在原数组中的相对顺序；后续任一次新增、metadata 更新、重排或删除都会把该服务器保留的记录从零开始连续重编号。

该顺序不再是每次进入项目就刷新的 MRU。首次加入此前不存在的项目时插入顶部，因此新添加项目优先；再次进入已有项目、返回其项目首页、直接打开其会话或重命名项目，只确保记录存在或更新 metadata，不改变位置。这些辅助 add/upsert 写入仍由应用级 `RecentProjectRecorder` 串行处理，并在导航后 best-effort 执行；本地持久化失败不能阻止进入本来可访问的项目，也不能回滚已经成功的重命名。

所有目录继续按 `serverId` 加平台感知的路径比较 key 规范化和去重。当前活动项目不在持久列表中时，rail 将其合入顶部，同时保持合并结果最多 20 项；通过重排 API 提交该展示顺序时，只能在同一次原子持久化中纳入这个被显式标识的活动项目。提交时，陈旧界面快照中已被并发删除的项目不会被重新创建；当前持久数据中未出现在陈旧快照里的记录（包括并发新增项目）会保留。在 rail 展示中，活动项目的实时 metadata 会替换匹配的旧记录，不会产生重复项。

项目按钮放在有界 `LazyColumn` 中，可见高度最多为抽屉安全内容高度的 75%；超出后纵向滚动，“打开项目”和“设置”固定保持可达且不参与重排。长按项目按钮启动带边缘自动滚动的纵向拖拽重排；展示顺序先乐观更新，原子 DataStore 写入失败时回滚到持久顺序。Rail 项目拖拽期间禁用外层 `ModalNavigationDrawer` 水平手势，避免纵向排序与抽屉关闭手势竞争。

每个项目都有 48dp 触控目标，显示第一个 Unicode 字符簇，提供包含完整项目名和路径的本地化标签，并在同一 `selectableGroup` 中使用 `Role.Tab`，且仅有一个选中项。项目选择页卡片正文和 Drawer 项目目标都提供本地化 TalkBack“上移项目/下移项目”自定义操作；Drawer 在增加这些操作的同时保留 Tab role、selected 状态、完整名称/路径标签和通知状态语义。只有活动项目显示当前可获得的通知状态；对尚未加载的非活动项目，不伪造未读、权限或工作中标记。

选择 rail 项目时先关闭抽屉。已存在于持久列表中的项目保持原位置；仍未持久化、仅临时合入的活动项目会按新添加项目记录。若当前已位于该项目首页，不增加重复目的地；若位于该项目会话详情，则返回项目首页。选择其他项目时优先弹回栈内已有的匹配项目首页，否则打开新的 `ProjectShellRoute`。Rail 进入项目首页，不恢复 Web 客户端的最近会话，因为最近项目持久化记录不包含 session ID。
...
项目主页和抽屉中的工作中会话行使用共享的 OpenCode 风格 4×4 运行指示器：四个角保持为空，其余点按相互独立的确定性节奏脉冲。Build 和 Plan 保留各自语义色，每一帧都满足 3:1 图形对比度下限，关闭系统动画后仍保留清晰可见的静态指示器。项目主页中，工作中状态优先于行尾通知圆点；运行中的子代理卡片复用同一指示器。
...
- 长按文件树或搜索结果中的任意文件/目录行会打开可聚焦复制菜单，不触发打开、展开或选择。文件提供“复制文件名”，目录提供“复制目录名”，两者均提供“复制相对路径”和“复制绝对路径”。复制路径统一使用 `/` 分隔符；绝对路径只由规范化项目根与已校验相对路径构造。
...
Android 建议：消息列表使用 `LazyColumn(reverseLayout = false)`，消息 action 在长按、更多菜单或正文下方操作区中展示，复制使用 Android Clipboard。会话正文按“逐条消息”启用文字选择：用户消息正文用 Compose 文本选择，助手 Markdown 段落用可选择 `TextView`，代码块用 Compose 选择容器；角色、模型、状态和时间不进入选择范围，避免复制噪声。异常提示文本使用独立选择容器，且当消息正文为空时“复制回复”复制异常提示。保留“复制消息/复制回复”整条复制作为兜底。在消息视口右下角叠放两个 48dp 跳转控件：一个定位到真正的第一条用户消息，另一个定位到最新消息或正在显示的思考项；手动到达当前已加载顶部时取一页更早消息，点击第一条用户消息控件时先取完所有剩余历史再定位。控件须位于 Composer、各类 Dock 和 IME 上方，并通过足够的底部滚动留白保证最后一项可完全露出。平台文字选择 ActionMode 激活时，控件以短暂淡出加缩放动画隐藏；退出选择后恢复显示，且不得强制把列表补滚到底部。用户消息的 `重置到此点` 对应 `POST /session/{sessionID}/revert`。agent/助手回复正文使用 Markdown 渲染，至少覆盖段落、列表、链接、行内代码和代码块；行内代码按 Web 端使用透明背景的青色等宽高亮文字，不做灰底 chip；fenced code block 使用接近 Web 的浅色背景、1dp 边框、等宽字体、横向滚动和语法高亮；用户消息仍按普通文本展示，避免输入内容被误解释。

会话历史 `GET /session/{sessionID}/message` 固定按 `limit=200` 分页；响应的 `X-Next-Cursor` 作为下一页更早历史的 `before`，无 cursor 表示历史完整。Store 按会话保存该状态，并把旧页前插合并，不能替换较新的 REST、SSE 或乐观数据。刷新后的第一页若与上一次第一页的 message id 有重叠，则保留已加载历史的最旧 cursor；若两页不相交，则从刷新响应的新 cursor 重新推进，让后续加载补齐中间断层，即使因此需要重新获取旧页。更早页响应只有在其请求的 `before` 仍等于 Store 当前最旧 cursor 时才应用；响应重复当前 cursor 或返回已消费 cursor 时直接失败，且不修改消息或 cursor 状态。该 transport 不经过 Retrofit converter 或 Retrofit 的错误体交付路径，而由 `ServerConnection` 中与普通 REST 共享同一个认证、超时、重定向、脱敏和 encoded-body 限制配置的 OkHttp client 直接构造每页请求。非 2xx 响应不读取 body，立即关闭并抛出只包含状态码的类型化 HTTP 异常。Network interceptor 对每一页在 HTTP transfer framing 之后、`Content-Encoding` 解码之前的 encoded response-body octets 执行 64 MiB 上限；2xx 响应再在 OkHttp callback 线程使用独立的 64 MiB decoded-entity 计数/限流 `InputStream`，通过应用统一的宽容 `Json.decodeFromStream` 直接解码 DTO，并在解码结束后继续经同一 limiter 消费到 EOF。未知长度、低报长度、尾随数据和无限流都在对应上限的第 `max + 1` 字节失败；协程取消会立即取消 Call 并关闭正在阻塞读取的 body，取消导致的 I/O 异常不能替换 `CancellationException`。这些逐页上限不等于累计历史、DTO、字符串、Base64 和 UI 映射的实际 heap 峰值，也不代表其他 REST 大响应已完成审计。

每个第一页请求都会注册 Store 共享 generation，只有最后注册的 generation 可以应用；旧详情刷新或恢复快照的迟到响应不能修改消息或 cursor 状态。消息顺序遵循服务端 `(createdAt, id)` 复合 key。第一页刷新期间若消息 revision 未变化，则校准其覆盖的最新窗口，使断线期间删除的消息或 part 消失，同时保留严格更早的已加载历史和乐观消息。若 revert 投影或其他过滤使已加载时间线为空但仍有更早 cursor，则取一页旧历史；即使没有最新可见项，也继续提供第一条用户消息控件。
...
frpc STCP visitor 首版只支持本地端口模式：App 启动 STCP visitor，绑定 `127.0.0.1:<本地绑定端口>`，再访问该本地地址。`:frpc-stcp-visitor` 模块暴露统一的 backend-neutral client contract，保留六个 suspend 操作：`startSession`、`ensureVisitor`、`waitVisitorReady`、`stopVisitor`、`stopSession` 和 `getState`。构建期选择让正式 `debug`、`release` 固定使用 GoMobile backend，内部 `canary` 与不可发布的 `kotlinRelease` Release-Like 真机测试构建固定使用纯 Kotlin backend。`kotlinRelease` 继承正式 Release 配置，增加 application ID suffix `.kotlinrelease` 与 version name suffix `-kotlin-release`，并使用标准 Android Debug 测试证书；它仅用于 Release 模式真机验证，绝不接受 Release 签名、暂存或发布。不提供用户可见设置、持久化 selector，也不在活动 generation 内自动 fallback，切换 backend 必须完整停止旧 session。两个实现共享 listener ownership、readiness revision、generation、control epoch、取消、有界 I/O、类型化失败以及公共 `useEncryption`/`useCompression` 语义。活动 listener 对端口保持独占；完整停止后，Kotlin listener 只启用 address reuse、不启用 `SO_REUSEPORT`，因此已完成 relay 的连接处于 `TIME_WAIT` 时，下一 generation 仍能快速重绑同一端口，但 backend 或 generation 绝不能重叠运行。Kotlin `stopVisitor` 会等待本地 listener 与 relay endpoint 失效。完整 relay pump 收尾、延迟的 best-effort Yamux reset 和 stream permit 释放继续由 session watcher 持有，并由 `stopSession` 在进入终态前 join。Kotlin library 会在 visitor 握手后通过有界、兼容 frp 的 AES-CFB/Snappy 流支持四种 payload 组合；当前 App schema 与 UI 不暴露这两个字段，继续提交其默认 `false` 值。
...
- 未读、错误、权限、连接、选中和工作中状态不只依赖颜色，使用文字、图标、边框、标记或状态说明表达相同含义。可排序服务器卡片和项目除长按拖拽外，还提供本地化 TalkBack 上移/下移自定义操作。
...
| GET | `/session/{sessionID}/message?limit=200&before={cursor?}` | cursor 分页消息列表；读取 `X-Next-Cursor`，绕过 Retrofit 错误体缓存，每个 2xx 页以流式 JSON decode 限制为 64 MiB，非 2xx 不读取 body |
...
| 服务器/项目排序只支持拖拽 | 长按拖拽不满足 TalkBack 使用 | 增加本地化上移/下移自定义操作 |
| 抽屉拖拽手势冲突 | 项目纵向排序可能与外层抽屉水平手势竞争 | rail 项目拖拽期间禁用外层抽屉手势；固定操作保持在重排列表外 |
...
| 项目选择页和最多 20 个项目的 Drawer | 长按纵向拖拽、边缘自动滚动、两个入口同步、普通项目点击、项目选择页删除图标、模拟持久化失败、当前项目不在持久列表、“打开项目”/“设置”和抽屉水平手势 | 两个入口展示同一持久顺序；新项目默认置顶；点击已有项目不重排；只有项目选择页卡片正文启动拖拽且删除保持独立；乐观移动失败后回滚；活动项目合入后仍最多 20 项且可通过重排持久化；固定操作不移动；仅 rail 拖拽期间禁用水平抽屉手势 |
| 紧凑手机上打开 IME | 底部 Composer、内联建议、model/agent/variant popup、附件条、系统返回 | Composer 保持在 inset 上方；浮层不被遮挡；系统返回先关闭聚焦浮层，再离开页面 |
| 启用 TalkBack | 图标标签、Tab/选择控件、展开/折叠控件、未读/错误/权限/连接状态、服务器与项目排序 | 正确播报 role 与状态；重要含义不只依赖颜色；服务器卡片和两个项目排序入口都提供上移/下移操作；Drawer 项目保留 `Role.Tab`、selected 状态、完整名称/路径和通知语义 |
...
| 最近项目排序 | 共享按服务器 DataStore 顺序、数值 `sortOrder`、项目选择页与 Drawer 中带边缘自动滚动的长按纵向拖拽、带失败回滚的乐观持久化，以及位于重排列表外的 Drawer 固定操作 |
| 无障碍语义 | 48dp 目标、选择 role、本地化展开/折叠说明、非颜色状态提示和 TalkBack 服务器/项目排序操作 |
...
| 项目 | 状态 | 当前验收说明 |
| --- | --- | --- |
| 最近项目 | 已具备 | 每个服务器最多保留 20 个规范化且按比较 key 去重的项目，按数值 `sortOrder` 升序展示；旧数组在后续连续重编号写入前保持顺序；新项目置顶，普通进入不移动已有项目，项目选择页/Drawer 的长按或 TalkBack 重排共享同一乐观顺序并支持失败回滚 |
...
| STCP 并发与恢复 | 主体具备 | readiness、generation/control epoch、AAR 门禁和扩展后的真实 Android GoMobile→Kotlin A/B 自动化均已实现。固定 API 26 x86_64 `compat` lane 只含 `success-v1-00`；API 36 x86_64 `full` lane 包含精确 12-profile wire/payload/negative/restart 矩阵，并由严格 Host summary 校验 backend 顺序、参数、语义结果、checks 与等价性。精确候选 `01ba5437276fb5074da5c654219668ac4cb69d48` 已在 [run 29763640048](https://github.com/ycfeng/ocdeck-android/actions/runs/29763640048) 通过普通完整验证 job、两条 Android lane 及 fail-closed 双语报告。当前已提供不可发布的 Kotlin Release-Like APK，用于不改变正式发布选择的纯 Kotlin Release 模式设备验证。pre-K7 接受分离证据矩阵：物理 ARM 覆盖目标 ABI 安装/class loading 和真实 App/STCP；官方 Android 16KB 测试镜像或真机覆盖 runtime page size、启动和完整互操作，并结合静态对齐与打包门禁。两个 lane 不要求使用同一台物理设备；无法获得物理 ARM 16KB 硬件属于残余风险和后续增强项，不再阻塞开发。Store/快照/reconciliation 与恢复、Doze/网络与前后台切换、性能、资源泄漏、长期 soak 和正式稳定发布周期证据仍须完成；任一单独 lane 或验证 variant 都不授权 Kotlin 默认装配 |
...
| 优先级 | 内容 |
| --- | --- |
| 高 | 完成 pre-K7 分离设备矩阵：在可用 ARM 真机验证目标 ABI 安装、Release 模式 class loading 和真实 App/STCP；在官方 Android 16KB 测试镜像或真机验证 `16384` runtime page size、启动和完整互操作。二者可以使用不同设备，缺少物理 ARM 16KB 设备不阻塞 App Store/快照/reconciliation、生命周期、性能、资源泄漏和长期 soak 工作；正式稳定发布周期另行继续验证。该 APK 不具备发布资格，也不能单独授权 Kotlin 默认决策 |
