...
| STCP concurrency and recovery | Core available | Readiness, generation/control epoch, AAR gates, and a phase-one real Android GoMobile-to-Kotlin A/B harness exist; a manual exact-SHA API 26/API 36 x86_64 workflow carries the emulator gate. Physical target ABIs, 16KB devices, the full wire/payload/negative/restart matrix, network/foreground transitions, performance, resource-leak, and soak evidence remain required |
...
| Priority | Work |
| --- | --- |
| High | Obtain and review a passing exact-SHA API 26/API 36 K6V report, then expand Android STCP evidence to physical ARM/16KB devices, wire/payload/restart paths, performance, resource-leak, and soak testing before any Kotlin-default decision |
