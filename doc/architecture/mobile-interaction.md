...
| Priority | Work |
| --- | --- |
| High | Using the recorded exact-SHA API 26 `compat`/API 36 `full` K6V pass as the emulator baseline, use the available non-publishable Kotlin Release-Like APK to validate physical ARM/16KB devices, Release-mode class loading, the App's real Store/snapshot/reconciliation and recovery paths, Doze/network and foreground/background transitions, performance, resource leaks, and long-duration soak. Continue formal stable-release-cycle validation separately; the APK is not publishable and does not authorize a Kotlin-default decision |
