...
The goal is to provide implementable page, interaction, state, API, and mobile-adaptation designs for reproducing the core OpenCode Web UI workflow in OC Deck. OC Deck is an independent community client and is not an official product of or affiliated with OpenCode or Anomaly. Application version `0.2.1` uses `io.github.ycfeng.ocdeck`, supports Android API 26 and later, and requires users to provide an accessible OpenCode Server. This document contains no real provider API keys, tokens, passwords, or local credentials. Sensitive fields in configuration APIs must be replaced with `<redacted>` in logs, documentation, crash reports, and debugging UI.
