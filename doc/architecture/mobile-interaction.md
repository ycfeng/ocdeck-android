...
| STCP concurrency and recovery | Core available | Readiness, generation/control epoch, AAR gates, and the expanded real Android GoMobile-to-Kotlin A/B automation are implemented. The fixed API 26 x86_64 `compat` lane contains only `success-v1-00`; the API 36 x86_64 `full` lane contains the exact 12-profile wire/payload/negative/restart matrix, with strict Host summary checks for backend order, parameters, checks, and equivalence. A new exact-SHA workflow artifact for these lanes is still pending. Candidate `459c2b57ebf465d6b933ea939f59fa739128ec59` and run 29716724485 remain old phase-one historical evidence only and do not prove the current `full` suite. Physical target ABIs, 16KB devices, the App's real Store/snapshot/reconciliation path, Doze/network and foreground/background transitions, performance, resource-leak, long-duration soak, and formal stable-release-cycle evidence remain required |
