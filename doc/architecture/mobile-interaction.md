...
| STCP concurrency and recovery | Core available | Readiness, generation/control epoch, AAR gates, and the expanded real Android GoMobile-to-Kotlin A/B automation are implemented. The fixed API 26 x86_64 `compat` lane contains only `success-v1-00`; the API 36 x86_64 `full` lane contains the exact 12-profile wire/payload/negative/restart matrix, with strict Host summary checks for backend order, parameters, semantic outcomes, checks, and equivalence. Exact candidate `c474ccd18769544a3d7a08facf4147dfe6305bee` passed both lanes and the bilingual fail-closed report in [run 29754014231](https://github.com/ycfeng/ocdeck-android/actions/runs/29754014231). Physical target ABIs, 16KB devices, the App's real Store/snapshot/reconciliation path, Doze/network and foreground/background transitions, performance, resource-leak, long-duration soak, and formal stable-release-cycle evidence remain required; the emulator report does not authorize Kotlin-default assembly |
...
| Priority | Work |
| --- | --- |
| High | Using the recorded exact-SHA API 26 `compat`/API 36 `full` K6V pass as the emulator baseline, validate physical ARM/16KB devices, the App's real Store/snapshot/reconciliation path, Doze/network and foreground/background transitions, performance, resource leaks, long-duration soak, and formal stable release cycles before any Kotlin-default decision |
