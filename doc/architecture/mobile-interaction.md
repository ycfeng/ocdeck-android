...
The goal is to provide implementable page, interaction, state, API, and mobile-adaptation designs for reproducing the core OpenCode Web UI workflow in OC Deck. OC Deck is an independent community client and is not an official product of or affiliated with OpenCode or Anomaly. Application version `0.2.3` uses `io.github.ycfeng.ocdeck`, supports Android API 26 and later, and requires users to provide an accessible OpenCode Server. This document contains no real provider API keys, tokens, passwords, or local credentials. Sensitive fields in configuration APIs must be replaced with `<redacted>` in logs, documentation, crash reports, and debugging UI.
...
The project picker and Android rail observe the same server-scoped persistent order, with at most 20 projects per server. Each serialized DataStore record has a numeric `sortOrder` and is displayed in ascending numeric order. Legacy records without `sortOrder` retain their relative array order; the next add, metadata update, reorder, or removal rewrites that server's retained records with continuous `sortOrder` values starting at zero.

This order is not an MRU that refreshes whenever a project is entered. Adding a previously unknown project inserts it at the top, so newly added projects appear first. Entering an existing project, returning to its project home, opening one of its sessions directly, or renaming it only ensures the record exists or refreshes its metadata without changing its position. These auxiliary add/upsert writes remain serialized through the application-scope `RecentProjectRecorder` and run best-effort after navigation; local persistence failure cannot block an otherwise accessible project or roll back a successful rename.

Every directory remains normalized and deduplicated by `serverId` plus the platform-aware path comparison key. When the active project is absent, the rail merges it at the top while keeping the merged list capped at 20; submitting that displayed order through the reorder API can atomically incorporate only that explicitly identified active project into persistence. At commit time, stale submitted entries that were concurrently deleted are not recreated, while current records omitted by a stale UI snapshot, including concurrently added projects, are retained. In the rail display, the active project's live metadata replaces a stale matching record without creating a duplicate.

Project buttons live in a bounded `LazyColumn` whose visible height is at most 75% of the safe drawer content height. Overflow scrolls vertically, while Open Project and Settings remain fixed, reachable, and excluded from reordering. Long-pressing a project button starts vertical drag reordering with edge auto-scroll. The displayed order updates optimistically and rolls back to the persisted order if the atomic DataStore write fails. While a rail item is being dragged, the outer `ModalNavigationDrawer` horizontal gesture is disabled so vertical ordering does not compete with drawer dismissal.

Each 48 dp project target uses its first Unicode character cluster, exposes a localized full project-name-and-path label, participates in one `selectableGroup` with `Role.Tab`, and has exactly one selected item. Both the picker card body and drawer project target expose localized TalkBack Move Project Up and Move Project Down custom actions. The drawer retains its tab role, selected state, full name/path label, and notification state semantics while adding those actions. Only the active project shows the currently available notification state; the app does not fabricate unread, permission, or working indicators for inactive projects that are not loaded.

Selecting a rail project closes the drawer first. A project already present in persistence keeps its saved position; a temporarily merged active project that is still absent from persistence is recorded as a newly added project. Selecting the project already visible on its project home does not add another destination; selecting it from a session detail returns to that project home. Selecting another project pops back to an existing matching project-home destination when possible, otherwise it opens a new `ProjectShellRoute`. The rail enters the project home rather than restoring the Web client's last session because recent-project persistence does not contain a session ID.
...
- Long-pressing any file or directory row in the tree or search results opens a focusable copy menu without triggering open, expand, or selection. Files expose Copy File Name, directories expose Copy Directory Name, and both expose Copy Relative Path and Copy Absolute Path. Copied paths use `/` separators; absolute paths are derived only from the normalized project root plus the validated relative path.
...
Android recommendation: render messages with `LazyColumn(reverseLayout = false)`. Put message actions in long press, overflow, or an action area below the content, and copy through Android Clipboard. Enable text selection per message: Compose selection for user text, selectable `TextView` blocks for assistant Markdown paragraphs, and Compose selection containers for code blocks. Exclude role, model, status, and time metadata from selection to avoid copy noise. Error text uses its own selection container; when response content is empty, Copy Response copies the error text. Keep whole-message Copy Message/Copy Response as a fallback. Overlay two 48 dp jump controls at the lower-right of the message viewport: one targets the true first user message and the other targets the latest message or active thinking item. Reaching the loaded top manually fetches one older page; using the first-user control fetches all remaining older pages before scrolling to the target. Keep the controls above the Composer, Docks, and IME, reserve enough bottom scroll padding to expose the final item, and hide them with a short fade-and-scale transition while a platform text-selection action mode is active. Exiting selection restores the controls without forcing the list back to the bottom. `Reset to here` on a user message maps to `POST /session/{sessionID}/revert`. Render agent/assistant responses as Markdown supporting at least paragraphs, lists, links, inline code, and fenced code blocks. Match Web inline code with cyan monospace text on a transparent background rather than a gray chip. Fenced blocks use a light Web-like background, 1 dp border, monospace font, horizontal scrolling, and syntax highlighting. User messages remain plain text so user input is not interpreted as Markdown.

Session history from `GET /session/{sessionID}/message` uses pages of `limit=200`. The response's `X-Next-Cursor` is sent as `before` for the next older page; a missing cursor marks complete history. The Store keeps this state per session and prepends older pages without replacing newer REST, SSE, or optimistic data. A refreshed first page that overlaps the previous first-page IDs preserves the already loaded oldest cursor; a disjoint refresh restarts from the new cursor so later loads bridge the missing window, even when that requires refetching old pages. An older-page response applies only while its requested `before` still equals the Store's current oldest cursor; a repeated or previously consumed cursor fails without changing messages or cursor state. The transport bypasses Retrofit converters and Retrofit's error-body delivery path. `ServerConnection` directly creates each page request with the same OkHttp client and therefore shares authentication, timeout, redirect, redaction, and encoded-body limiting with normal REST calls. For non-2xx responses, do not read the body; close immediately and throw a typed HTTP error containing only the status code. A network interceptor limits each page's encoded response-body octets after HTTP transfer framing and before `Content-Encoding` decoding to 64 MiB. For 2xx responses, decode DTOs directly on the OkHttp callback thread through the application's shared lenient `Json.decodeFromStream`, using a separate 64 MiB decoded-entity counting `InputStream` and a `Content-Length` fast rejection. After decoding, continue consuming through the same limiter to verify EOF. Unknown lengths, underreported lengths, trailing data, and infinite streams fail at the applicable limit plus one byte. Coroutine cancellation immediately cancels the Call and closes a body blocked in reading; resulting I/O failures must not replace `CancellationException`. These per-page limits are not the actual peak heap use of the accumulated history, DTOs, strings, Base64 data, and UI mappings, and they do not claim that other large REST responses have been audited.

Each first-page request registers a Store-shared generation, and only the latest generation may apply. A late response from an older detail refresh or recovery snapshot changes neither messages nor cursor state. Message order follows the server's `(createdAt, id)` key. When no message revision changed during a first-page refresh, reconcile the covered latest window so messages or parts deleted while disconnected disappear while strictly older loaded history and optimistic messages survive. If revert projection or another filter leaves the loaded timeline empty while an older cursor remains, fetch one older page and keep the first-user control available even without a latest visible item.
...
The first `frpc STCP visitor` version supports local-port mode only. The app starts the STCP visitor, binds `127.0.0.1:<local-bind-port>`, and accesses that local address. The `:frpc-stcp-visitor` module exposes one backend-neutral client contract with six suspend operations: `startSession`, `ensureVisitor`, `waitVisitorReady`, `stopVisitor`, `stopSession`, and `getState`. Build-time selection keeps formal `debug` and `release` on the GoMobile backend, while internal `canary` and the non-publishable `kotlinRelease` Release-Like device-test build use the pure Kotlin backend. `kotlinRelease` inherits the formal Release configuration, adds application ID suffix `.kotlinrelease` and version-name suffix `-kotlin-release`, and uses the standard Android Debug test certificate; it exists only for Release-mode device validation and is never release-signed, staged, or published. There is no user-visible setting, persisted selector, or automatic fallback within an active generation; switching backend requires fully stopping the old session. Both implementations share listener ownership, readiness revision, generation, control-epoch, cancellation, bounded-I/O, typed-failure, and public `useEncryption`/`useCompression` semantics. An active listener keeps exclusive ownership of its port. After complete stop, the Kotlin listener enables address reuse, but not `SO_REUSEPORT`, so the next generation can quickly rebind the same port despite completed relay connections in `TIME_WAIT`; backends or generations must never overlap. Kotlin `stopVisitor` waits for the local listener and relay endpoints to become unavailable. Full relay-pump completion, delayed best-effort Yamux reset, and stream-permit release remain owned by a session watcher that `stopSession` joins before terminal close. The Kotlin library supports all four payload combinations after the visitor handshake with bounded frp-compatible AES-CFB/Snappy streams. The current App schema and UI do not expose these fields and continue to submit their default `false` values.

While the GoMobile backend remains in formal builds, the repository keeps the `frpc-stcp-visitor-go/` wrapper, pinned versions, and auditable downstream patch. Local scripts or CI generate its immutable-version AAR without committing it, and Android consumes it through a local Maven repository. Release must verify AAR checksum, API signature, provenance, expected ABIs, stripped state, and 16 KB ELF alignment. On the GoMobile path, missing generated bridge classes produce `GoMobileBridgeUnavailableException`, while incompatible bridge classes, methods, or payloads produce `GoMobileBridgeApiMismatchException`. These and pure Kotlin backend failures remain typed and payload-free; STCP readiness and SSE map permanent setup failures to localized semantic errors instead of exposing reflection, protocol, or sensitive details. If the selected backend is unavailable, the UI may save configuration, but connection attempts must return a clear unavailable error.
...
- Unread, error, permission, connection, selected, and working states do not rely on color alone. Text, icons, borders, markers, or state descriptions carry the same meaning. Reorderable server cards and projects provide localized TalkBack Move Up and Move Down custom actions in addition to long-press drag.
...
| GET | `/session/{sessionID}/message?limit=200&before={cursor?}` | Cursor-paged message list; read `X-Next-Cursor`, bypass Retrofit error-body caching, stream-decode each 2xx page with a 64 MiB limit, and do not read non-2xx bodies |
...
| Drag-only server/project ordering | Long-press drag is not sufficient for TalkBack | Add localized Move Up/Move Down custom actions |
| Drawer drag gesture conflict | Vertical project ordering can compete with the outer horizontal drawer gesture | Disable outer drawer gestures while a project rail item is being dragged; keep fixed actions outside the reorder list |

### 18.1 Manual UI Validation Matrix

The existing `app/src/androidTest` coverage is limited to localized independent-window roots. Ordinary CI has no App UI emulator/instrumentation job; the separate manual K6V workflow automates only STCP backend interoperability on x86_64 emulators. Recent-project ordering and the rest of this UI/TalkBack matrix therefore remain manual device checks:
...
| Project picker and drawer with up to 20 projects | Long-press vertical drag, edge auto-scroll, picker/drawer synchronization, ordinary project click, picker delete icon, forced persistence failure, active project missing from persistence, Open Project/Settings, and the drawer's horizontal gesture | Both entries show the same saved order; a new project starts at the top; clicking an existing project does not reorder; only the picker card body starts drag and Delete remains independent; optimistic movement rolls back on failure; the active-project merge remains capped at 20 and can persist through reorder; fixed actions never move; horizontal drawer gestures are disabled only during rail drag |
...
| TalkBack enabled | Icon labels, tabs and selection controls, expanded/collapsed controls, unread/error/permission/connection states, server and project ordering | Roles and states are announced; important meaning is not color-only; server cards and both project-ordering surfaces expose Move Up/Move Down actions; drawer projects retain `Role.Tab`, selected state, full name/path, and notification semantics |
...
| Recent-project ordering | Shared server-scoped DataStore order, numeric `sortOrder`, long-press vertical drag with edge auto-scroll in picker and drawer, optimistic persistence with rollback, and fixed drawer actions outside the reorder list |
| Accessibility semantics | 48 dp targets, selection roles, localized expanded/collapsed descriptions, non-color status cues, and TalkBack server/project reorder actions |
...
| --- | --- | --- |
| Recent projects | Available | Keeps at most 20 normalized, comparison-key-deduplicated projects per server in ascending numeric `sortOrder`; legacy arrays retain order until the next continuous renumbering write; new projects start at the top, ordinary entry does not move existing projects, and picker/drawer long-press or TalkBack reordering shares one optimistically updated order with rollback |
...
| STCP concurrency and recovery | Core available | Readiness, generation/control epoch, AAR gates, and the expanded real Android GoMobile-to-Kotlin A/B automation are implemented. The fixed API 26 x86_64 `compat` lane contains only `success-v1-00`; the API 36 x86_64 `full` lane contains the exact 12-profile wire/payload/negative/restart matrix, with strict Host summary checks for backend order, parameters, semantic outcomes, checks, and equivalence. Exact candidate `01ba5437276fb5074da5c654219668ac4cb69d48` passed the complete ordinary verification job, both Android lanes, and the bilingual fail-closed report in [run 29763640048](https://github.com/ycfeng/ocdeck-android/actions/runs/29763640048). A non-publishable Kotlin Release-Like APK is available for Release-mode device validation without changing formal release selection. Pre-K7 accepts a split evidence matrix: physical ARM covers target-ABI installation/class loading and real App/STCP behavior, while an official Android 16KB test image or physical device covers runtime page size, startup, and full interoperability with static alignment and packaging gates. The lanes need not share one physical device; unavailable physical ARM 16KB hardware is a residual risk and follow-up, not a development blocker. Store/snapshot/reconciliation and recovery, Doze/network and foreground/background transitions, performance, resource-leak, long-duration soak, and formal stable-release-cycle evidence remain required; no individual lane or verification variant authorizes Kotlin-default assembly |
...
| Mobile UI and accessibility | Core available | 48 dp targets, matching selection roles, localized expanded/collapsed descriptions, non-color status cues, TalkBack server/project reorder actions, light/dark contrast tests, and bounded large-text/IME layouts are implemented; App UI device instrumentation is not yet a CI gate |
...
| Priority | Work |
| --- | --- |
| High | Complete the split pre-K7 device matrix: validate target-ABI installation, Release-mode class loading, and real App/STCP behavior on available physical ARM devices; validate a `16384` runtime page size, startup, and full interoperability on an official Android 16KB test image or physical device. These may be different devices, and lack of a physical ARM 16KB device does not block App Store/snapshot/reconciliation, lifecycle, performance, resource-leak, and long-duration soak work. Continue formal stable-release-cycle validation separately; the APK is not publishable and does not by itself authorize a Kotlin-default decision |
