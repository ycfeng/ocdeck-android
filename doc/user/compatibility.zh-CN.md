...
| STCP bridge 制品 | 固定 GoMobile bridge `0.3.8-frp0.69.1-p1` | Tested | 自动门禁校验 checksum、Java API、provenance、四 ABI Go BuildInfo 中固定且版本化的 module identity/sum、无本地路径、唯一 module graph digest、ELF machine、16KB `PT_LOAD` 对齐、stripped 状态，以及隔离 cache 后在同一平台不同 checkout 路径间的逐字节可复现性。 |
