...
- SSE line 或 event 超过 32 MiB wire-byte 上限。
...
4. 在系统设置中确认应用通知和“会话通知”channel 已启用。
...
低于 API 33 的 Android 不需要运行时通知权限，但系统级应用或 channel 设置仍可能抑制通知。电池优化豁免只能降低中断概率，不能保证进程死亡后继续送达。
