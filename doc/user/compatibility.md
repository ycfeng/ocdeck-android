...
| STCP bridge artifact | Fixed GoMobile bridge `0.3.11-frp0.69.1-p1` | Tested | Automated gates verify checksum, Java API, provenance, four-ABI Go BuildInfo with fixed versioned module identities/sums and no local paths, one module-graph digest, ELF machine, 16KB `PT_LOAD` alignment, stripped state, and same-platform byte reproducibility across distinct checkout paths with isolated caches. |
